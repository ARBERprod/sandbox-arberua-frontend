...
import { getPayPlaceForm } from '@/features/CheckoutForm';
import { Typography } from '@/shared/ui/Typography';
import { Loader } from '@/shared/ui/Loader';

interface PayPlacePageProps {
  url: string | null;
  isError: boolean;
}
...
const PayPlacePage = ({ url = '', isError }: PayPlacePageProps) => {
...
export const getServerSideProps = wrapper.getServerSideProps<PayPlacePageProps>(
  (store) => async ({ query, locale }) => {
    const orderId = query.order_id as string;

    const { data: url, isError } = await store.dispatch(
      getPayPlaceForm.initiate({ orderId }),
    );
